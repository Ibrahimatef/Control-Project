...
S=str2num(get(handles.edit18, 'string'));
T=str2num(get(handles.edit19, 'string'));
fileID3 = fopen('AB.h','w');
fprintf(fileID3, '#ifndef _AB_H_\n');
fclose(fileID3);
fileID3 = fopen('AB.h','a');
fprintf(fileID3, '#define _AB_H_\n');
fprintf(fileID3, 'extern const float A[');%Ap
fprintf(fileID3, num2str(length(Ap)));
fprintf(fileID3, '];\n');
...
fprintf(fileID2, 'const float R[]=\t');%R
fprintf(fileID2, '{\t');
for k=1:length(R)
     if k==length(R)
...
fprintf(fileID2, '}\n');
fprintf(fileID2, 'const float s[]=\t');%s
fprintf(fileID2, '{\t');
for k=1:length(S)
     if k==length(S)
...
fprintf(fileID2, '}\n');
fprintf(fileID2, 'const float T[]=\t');%T
fprintf(fileID2, '{\t');
for k=1:length(T)
    if k==length(T)
